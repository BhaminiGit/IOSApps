...
//  Untangler
//
//  Created by Bhamini Sundararaman on 7/9/21.
//
...
class ViewController: UIViewController {
    var currentLevel = 0
    //variable that stores current level

    var connections = [ConnectionView]() //stores the connectios
    ...
    override func viewDidLoad() {
     ...
     func levelUp(){
        currentLevel += 1 //go up a level
        
        //clear out the connection views we have
        connections.forEach{$0.removeFromSuperview() }
        connections.removeAll()
        //REMOVE EVERYTHING, AND WE CAN START FRESH
    ...
    func checkMove(){
        if levelClear(){
            ...
            view.isUserInteractionEnabled = false //stop any more dragging now
            ...
            //they are still plyaing this level
            score-=1
        }
    }
